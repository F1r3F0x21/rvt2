    ...
    if f.endswith('txt'):
        ...
        for section in module.sections():
            if 'SQL Query' in section and VERSION in section:
                secname = section
        if secname is None:
            print(f'Module {f}: no section found for version {VERSION}')
        else:
            # name = module.get('Query Metadata', 'QUERY_NAME')
            name = os.path.splitext(f)[0]

            # remember: $ character must be escaped as $$
            regex_rvt2 = '|'.join(list(map(lambda d: f'(.*/{d}$$)', module.get('Database Metadata', 'DATABASE').split(','))))
            secname_rvt2 = f'FileParser.ios.{name}'
