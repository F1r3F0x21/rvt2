...
class GetEvents(object):
    ...
    def __init__(self, eventfile, config_file, logger=logging):
        ...
        try:
            ...
            self.logger.warning(f'Configuration file {config_file} has not been properly loaded: {exc}')
            self.data_json = {}
    ...
    def parse(self):
        ...
        try:
            ...
            self.logger.warning(f'Error reading file {self.eventfile}. {exc}')
            return []
        ...
        try:
                ...
                # Selected events
                    ...
                    for field in ['category', 'type', 'action']:
                        if field in self.data_json[event_code]:
                            data[f'event.{field}'] = self.data_json[event_code][field]
            ...
            self.logger.warning(f'Error with pyevtx when reading the {self.count} event from {self.eventfile}: {exc}')
...
class EventJob(base.job.BaseModule):
    ...
    def get_evtx(self, path, regex_search):
        ...
        self.logger().debug(f'No evtx file found in {path} with name expression {regex_search}')
        return
...
class ParseEvents(EventJob):
    ...
    def run(self, path=None):
        ...
        self.logger().debug(f'Parsing event log {evtx_file}')
        if not evtx_file:
            return []
    ...
    def run(self, path=None):
        ...
        for evtx_filename in os.listdir(path):
            ...
            try:
                ...
                self.logger().warning(f'Problems parsing file {evtx_file}')
    ...
    def run(self, path=None):
        ...
        for ev in events_parser.parse():
            ...
            if ev['event.code'] == '45058':
                ...
                ev['message'] = f"A logon cache entry for user {ev['destination.user.name']} was the oldest entry and was removed. The timestamp of this entry was {ev['data.LastLoginLocalTime']}"
                ev.pop('user_date')
                ...
                for e, field in enumerate(fields[ev['event.code']]['fields']):
                    if data[e] == '(NULL)' or data[e] == '':
                        continue
                    ev[field] = data[e]
                    ev['message'] = ev['message'].replace(f'<{field}>', data[e])
            yield ev
        self.save_stats(events_parser.evtx_stats())
    ...
    def run(self, path=None):
        ...
        for ev in events_parser.parse():
            if "data.Reason" in ev.keys() and ev["event.code"] == "39":
                ev['data.ReasonStr'] = f'SessionID {ev["data.SessionID"]} disconnected by session {ev["data.Source"]}'
    ...
    def run(self, path=None):
        ...
        for ev in events_parser.parse():
            ...
            try:
                ...
                self.logger().warning(f"Skipping {self.config.job_name} event due to error")
                continue
        self.save_stats(events_parser.evtx_stats())
                    ...
                    for e, field in enumerate(fields[ev['event.code']]['fields']):
                        ...
                        if field == 'others':
                            for item in data[e][1:].split('\n'):
                                ...
                                ev['message'] = ev['message'].replace(f'<{aux_fields.group(1)}>', aux_fields.group(2))
                            continue
                        ev[field] = data[e]
                        ev['message'] = ev['message'].replace(f'<{field}>', data[e])
            yield ev
...
class WinFirewall(EventJob):
    """ Extracts events of parsed Microsoft-Windows-Windows Firewall With Advanced Security%4Firewall.evtx """

    def run(self, path=None):
        """
        Attrs:
            path (str): Absolute path to Microsoft-Windows-Windows Firewall With Advanced Security%4Firewall.evtx
        """
        path = self.get_evtx(path, r"/Microsoft-Windows-Windows Firewall With Advanced Security%4Firewall.evtx$")
        if not path:
            return []
        ...
        profiles = {1: 'Domain', 2: 'Private', 3: 'Private,Domain', 0x4: 'Public', 5: 'Public, Domain', 6: 'Public, Private', 7: 'Private, Domain, Public', 0x7fffffff: 'Private, Domain, Public', 0x80000001: 'None'}
        origin = {1: 'Local', 2: 'Group Policy', 3: 'Dynamic', 4: 'AutoGenerated', 5: 'Hardcoded'}
        direction = {1: 'Inbound', 2: 'Outbound'}
        action = {1: 'Authenticated IPSec Bypassk', 2: 'Block', 3: 'Allow'}
        protocol = {1: 'ICMP V4', 6: 'TCP', 17: 'UDP', 58: 'ICMP V6', 256: 'Any'}

        for ev in events_parser.parse():
            try:
                if 'data.Origin' in ev.keys():
                    ev['data.Origin'] = origin.get(int(ev['data.Origin']), ev['data.Origin'])
                if 'data.Direction' in ev.keys():
                    ev['data.Direction'] = direction.get(int(ev['data.Direction']), ev['data.Direction'])
                if 'data.Action' in ev.keys():
                    ev['data.Action'] = action.get(int(ev['data.Action']), ev['data.Action'])
                if 'data.Profiles' in ev.keys():
                    ev['data.Profiles'] = profiles.get(int(ev['data.Profiles']), ev['data.Profiles'])
                if 'data.Protocol' in ev.keys():
                    ev['data.Protocol'] = protocol.get(int(ev['data.Protocol']), ev['data.Protocol'])
                yield ev
            except Exception:
                self.logger().warning(f"Skipping {self.config.job_name} event due to error")
                continue
    ...
    def run(self, path=None):
        ...
        for evt_filename in os.listdir(path):
            ...
            if not evt_file.lower().endswith('.evt'):
                continue
            self.logger().debug(f'Parsing event log {evt_file}')
            try:
                ...
                self.logger().warning(f'Problems parsing file {evt_file}')
